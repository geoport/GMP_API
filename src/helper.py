...
def get_sa_pred(process_pipeline, Mw, Rrup, VS30, fault_type, period):
    df = pd.DataFrame(
        [[Mw, fault_type, Rrup, VS30, 1]],
        columns=["MW", "FaultType", "Rrup", "VS30", "SA"],
    )
    df_scaled = pd.DataFrame(
        process_pipeline.transform(df),
        columns=["Mw", "Rrup", "VS30", "SA", "FT_Ters", "FT_Yanal"],
    )
    model = joblib.load(f"models/SA_models/P{int(round(period*100,2))}.pkl")
    pred_scaled = model.predict(df_scaled.drop("SA", axis=1))
    scaler = process_pipeline.named_steps["col_trans"].transformers_[0][1]
    pred = scaler.inverse_transform([[1, 1, 1, pred_scaled]])[0][-1]
